...
const TodosContextProvider: FC<React.PropsWithChildren<{}>> = (props) => {
  const [todos, setTodos] = useState<Todo[]>([]);
...
- `createContext` is a generic function, meaning it accepts a type parameter to specify parameter to specify the context object's structure
- We only sets up the argument type, which describes context object
